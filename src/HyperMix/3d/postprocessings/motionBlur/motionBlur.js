...
function init(sampleCount, _fboHelper) {
  ...
  _linesMaterial = new THREE.RawShaderMaterial({
    ...
    vertexShader: fboHelper.rawShaderPrefix + motionBlurLinesvert,
    fragmentShader: fboHelper.rawShaderPrefix + motionBlurLinesfrag,
...
function resize(width, height) {
  ...
  if (!exports.useSampling) {
    const linesWidth = ~~(width * exports.linesRenderTargetScale);
    const linesHeight = ~~(height * exports.linesRenderTargetScale);
    _linesRenderTarget.setSize(linesWidth, linesHeight);

    let i;
    const noDithering = !exports.useDithering;
    const amount = noDithering ? linesWidth * linesHeight : _getDitheringAmount(linesWidth, linesHeight);
    const currentLen = _linesPositions.length / 6;
